...
EdibleCSS is a primitive CSS framework that makes your HTML pages look
professional with **zero effort**. No CSS classes, no configuration, no
JavaScript—just add one line of HTML and you're done.
...
Download `edible.min.css` from
[GitHub Releases](https://github.com/svmukhin/edible-css/releases)
and link it:
...
Your page automatically looks good on mobile, tablet, and desktop. No media
queries needed.

### ✅ Dark Mode

If your users have dark mode enabled in their OS/browser, your page
automatically switches to a dark theme. No toggle, no JavaScript—it just works.
...
EdibleCSS is **opinionated by design**. It has one style, no configuration.
That's the point.
...
EdibleCSS doesn't provide layout systems. Use semantic HTML (like `<nav>`,
`<main>`, `<article>`) and it will look fine. If you need complex layouts, this
isn't the framework for you.
...
**A:** Yes, but EdibleCSS doesn't provide any. It only styles HTML elements. You
can add your own classes for custom styling.
...
**A:** Because it makes your web pages look "edible" (good enough to serve to
users) without requiring design skills.
...
**A:** Then this framework isn't for you. EdibleCSS is opinionated. If
you want customization, try [Bootstrap](https://getbootstrap.com/) or
[Tailwind CSS](https://tailwindcss.com/).

### Q: How do I contribute?

**A:** Check the [GitHub repository](https://github.com/svmukhin/edible-css)
for contribution guidelines. Pull requests welcome!
...
1. **Forgot the viewport meta tag**: Add `<meta name="viewport"
   content="width=device-width, initial-scale=1.0">` to your `<head>`
2. **Invalid HTML**: EdibleCSS styles valid HTML5. Check your markup.
3. **Conflicting CSS**: Another stylesheet might be overriding EdibleCSS. Load
   EdibleCSS last.
...
3. **Firefox/Chrome**: DevTools → More tools → Rendering → Emulate CSS media
   feature prefers-color-scheme

### Content is too wide/narrow

EdibleCSS constrains content to 70ch (mobile), 75ch (tablet), or 80ch (desktop)
for readability. To override:
...
You're done! Go build something. EdibleCSS handles the design so you can focus
on your content.
